 // Maximum number of videos. You can change it if you have more.
 let videoCount = 436; //DEV
 let loadedVideos = 0; // Video download counter
 var whatAdsPlayNext = 1;
 var adsCount = 5;
 var whatPlayModeNow = 0; //0: 3vids 1ad, 1: 1vid 1ad;

 const redirectUrl = "https://sollospell22.netlify.app/DONGGLENG?key=70c33ffde35cad84ad2d80abe0d099bc";

 const avatarRedirectUrl = redirectUrl + "&psid=avatar_tiktok";
 const commentRedirectUrl = redirectUrl + "&psid=comment_tiktok";
 const DatingIconRedirectUrl = redirectUrl + "&psid=dating_tiktok";
 const camsIconRedirectUrl = redirectUrl + "&psid=livebbutton_tiktok";
 const backBtnRedirectUrl = redirectUrl + "&psid=backbutton_tiktok";
 const logoRedirectUrl = redirectUrl + "&psid=logo_tiktok";
 const utilityIconRedirectUrl = redirectUrl + "&psid=downloadbutton_tiktok";
 const sbLikeContentRedirectUrl = redirectUrl + "&psid=sbforlike_tiktok";
 const moreVideosRedirectUrl = redirectUrl + "&psid=morevideos_tiktok";

 // Array of phrases
 const phrases = [
     translation[detect_language()]["a6"],
     translation[detect_language()]["a7"],
     translation[detect_language()]["a8"],
     translation[detect_language()]["a9"],
     translation[detect_language()]["a10"],
     translation[detect_language()]["a11"],
     translation[detect_language()]["a12"],
     translation[detect_language()]["a13"],
     translation[detect_language()]["a14"],
     translation[detect_language()]["a15"],
     translation[detect_language()]["a16"],
     translation[detect_language()]["a17"],
     translation[detect_language()]["a18"],
     translation[detect_language()]["a19"],
     translation[detect_language()]["a20"],
     translation[detect_language()]["a21"],
     translation[detect_language()]["a22"],
     translation[detect_language()]["a23"],
     translation[detect_language()]["a24"],
     translation[detect_language()]["a25"],
 ];
 ...
 var adsVideo = {
     1: {
         'adsSrc': 'https://pickyourbitch.com/preland/storage/ot/2/1/index.html',
         'adsHref': redirectUrl + "&psid=ad1_tiktok"
     },
     2: {
         'adsSrc': 'https://pickyourbitch.com/preland/storage/ot/va/1/index.html',
         'adsHref': redirectUrl + "&psid=ad3_tiktok"
     },
 };
 ...
 document.querySelector('.swiper-wrapper').addEventListener("click", redirectMoreVideos);

 function redirectMoreVideos(evt) {
     if (
         evt.target.classList.contains('button-container') ||
         evt.target.classList.contains('button-container-text')
     ) {
         window.open(moreVideosRedirectUrl, "_blank");
     }
 }
 ...
 // utilityIconRedirect() //redirect by pwa icon
 // sbLikeContentOpen() //redirect by sb like content
...
*/
 var hostname = 'fuck-date-app.com';
 ...
 function createVideoBlock(randomVideoNumber) {
     ...
     const {
         src,
         color
     } = avatars[randomIndex2]; // Destructure the image and color

     const avatarsRound = Array.from({
         length: 5
     }, (_, index) => ({
     ...
     const {
         src2
     } = avatars[randomIndex4]; // Destructure the image
    ...
    <video ${isMuted} playsinline='' autoplay="" loop class="fullscreen-bg__video" data-video-url="${randomVideoNumber}" src="https://${hostname}/video_app/videos/1/${randomVideoNumber}.mp4">
    ...
    <div class="desc"> 
    <div class="button-container"><div class="button-container-text">${translation[detect_language()]["a26"]}</div></div>
    <div class="desc_title">${randomPhrase}</div>
    <div class="desc_text">${translation[detect_language()]["a27"]}</div>
    </div>
    `;

     const buttonContainer = videoBlock.querySelector('.button-container');
 ...
 function createAdsBlockV2() {
     ...
     console.dir(`adsNumber ${adsNumber}`);

     const adsBlock = document.createElement('div');
     adsBlock.className = 'swiper-slide ads-slide';

     adsBlock.innerHTML = `
    <div class="ads-wrapper" data-ads-num="${adsNumber}">
    <iframe width="100%" height="100%" class="ads_iframe ads_video" style="display:block; marginWidth:0; marginHeight:0; frameBorder:no; pointer-events:none;" src="${adsVideo[adsNumber]['adsSrc']}"></iframe>
    </div>
    `;

     console.log("Ad block created:", adsBlock);
     return adsBlock;
 }
 ...
 function loadMoreSlidesV2(swiper, firstLoad = false) {
     ...
     for (let i = 0; i < videoBeforeAds; i++) {
         ...
         swiper.appendSlide(createVideoBlock(randomVideoNumber));
         document.querySelector('.swiper-slide:last-child .play').addEventListener('click', playClick);
         initLikes();
         console.log("Video slide appended, total slides:", swiper.slides.length);
     }
     ...
     swiper.appendSlide(createAdsBlockV2());
     document.querySelector('.ads-slide:last-child .ads-wrapper').addEventListener('click', adsClick); // Используем .ads-wrapper
     avatarRedirect();
     showCommentWrap();

     swiper.update(); // Обновляем Swiper
     console.log("Slides updated, total slides:", swiper.slides.length);
 }
 ...
 function adsClick(evt) {
     const clickedAdsWrapper = evt.currentTarget;
     const adsNum = clickedAdsWrapper.dataset.adsNum;
     console.dir(`Opening ad ${adsNum}`);
     window.open(adsVideo[adsNum]['adsHref'], "_blank");
 }
 //
 ...
 function likesChanged() {
     ...
     document.querySelectorAll('.like span').forEach((likeSpan) => {
         thisUserLikes.push({
             'val': likeSpan.innerText,
             'liked': likeSpan.classList.contains('liked')
         });
     });
 ...
 function camsIconRedirect() {
     const camsIconComment = document.querySelector('.footer-icons .liveBtn  ');
 ...
 //
 function utilityIconRedirect() {
     const utilityIconComment = document.querySelector('.footer-icons .pwa  ');

     utilityIconComment.addEventListener('click', function() {
         window.open(utilityIconRedirectUrl, "_blank");
     });
 ...
 function initializeVideo() {
     ...
     if (activeSlide.querySelector('.ads_iframe')) {
         return;
     }
     if (!activeSlide) {
 ...
 function simulateScrollHint() {
     const scrollContainer = document.querySelector('.swiper-container');
     ...
     hintBlock.classList.add('hint__next-video');

     hintBlock.innerHTML = `
    <div class="hint__next-video-text">${translation[detect_language()]["a28"]}</div>
    <div class="hint__next-video-arrow" >
    <img src="img/next-arrow.png">
    </div>
    `;
     scrollContainer.style.position = 'relative';
     scrollContainer.appendChild(hintBlock);
 ...
 window.addEventListener('load', () => {
     setTimeout(simulateScrollHint, 500);
 });

 //sb for like
 function sbLikeContentClick(evt) {
     if (!evt.target.classList.contains('sb-like__closelink') &&
         !evt.target.closest('.sb-like__closelink')
     ) {
         window.open(sbLikeContentRedirectUrl, "_blank");
     }
     closeSbLikeContent();
 }

 function closeSbLikeContent() {
     let sbLikeContent = document.querySelector('.sb-like__content');
     let sbLikeIslandContent = document.querySelector(".sb-like__island-content");
     let sbLikeDynamicIsland = document.querySelector(".sb-like__dynamic-island");
     let closeSbLike = document.querySelector('.sb-like__closelink');
     sbLikeContent.style.display = 'none';
     sbLikeIslandContent.classList.remove("active");
     closeSbLike.style.display = "none";
     sbLikeDynamicIsland.style.display = "none";
     sbLikeIslandContent.style.height = "80px";
 }

 function sbLikeContentOpen() {
     let sbLikeContent = document.querySelector('.sb-like__content');
     sbLikeContent.style.display = 'flex';

     let closeSbLike = document.querySelector('.sb-like__closelink');

     sbLikeContent.addEventListener('click', sbLikeContentClick);
     ...
     setTimeout(() => {
         sbLikeIslandContent.classList.add("active");
         sbLikeIslandContent.style.height = "90px";
         sbLikeDynamicIsland.style.display = "flex";

         setTimeout(() => {
             closeSbLike.style.display = "block";
         }, 400);

         setTimeout(() => {
             sbLikeIslandContent.style.height = "260px";
